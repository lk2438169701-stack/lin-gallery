(function () {
  ...
  const SILK_DEFAULTS = { colorStops: ['#6e5b9b', '#f0c6dc', '#eeeae3'], speed: .55, softness: .8 };
  let backgroundMode = localStorage.getItem('lin-gallery-background-mode') || 'liquid';
  let silkSettings = (() => { try { return { ...SILK_DEFAULTS, ...(JSON.parse(localStorage.getItem('lin-gallery-silk') || 'null') || {}) }; } catch (_) { return { ...SILK_DEFAULTS }; } })();
  function activeBackgroundMode() { return backgroundMode; }
  function applyBackgroundMode(mode) {
    backgroundMode = ['liquid', 'aurora', 'silk'].includes(mode) ? mode : 'liquid';
    localStorage.setItem('lin-gallery-background-mode', backgroundMode);
  ...
  function createScene(THREE) {
    ...
    if (activeBackgroundMode() === 'silk') applySilkSettings();
  ...
  function bindUI() {
    ...
    ui.backgroundModes?.forEach((button) => button.addEventListener('click', () => { applyBackgroundMode(button.dataset.backgroundMode); if (ui.backgroundSwitcher) ui.backgroundSwitcher.hidden = true; if (backgroundMode === 'aurora' && ui.detailPanel?.classList.contains('is-open')) window.linAuroraBackground?.setVisible(true); }));
    ...
    [[ui.silkColor1, 0], [ui.silkColor2, 1], [ui.silkColor3, 2]].forEach(([input, index]) => input?.addEventListener('input', () => { silkSettings.colorStops[index] = input.value; localStorage.setItem('lin-gallery-silk', JSON.stringify(silkSettings)); applySilkSettings(); }));
    [['silkSpeed','speed'], ['silkSoftness','softness']].forEach(([control, key]) => ui[control]?.addEventListener('input', () => { silkSettings[key] = Number(ui[control].value); localStorage.setItem('lin-gallery-silk', JSON.stringify(silkSettings)); applySilkSettings(); }));
    ui.silkReset?.addEventListener('click', () => { silkSettings = { ...SILK_DEFAULTS, colorStops: [...SILK_DEFAULTS.colorStops] }; localStorage.setItem('lin-gallery-silk', JSON.stringify(silkSettings)); applySilkSettings(); });
