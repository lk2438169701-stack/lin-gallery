(function () {
  ...
  let detailBackgroundPalettes = (() => { try { const saved = JSON.parse(localStorage.getItem('lin-gallery-detail-background') || 'null'); return { light: { ...BACKGROUND_DEFAULTS.light, ...(saved?.light || {}) }, dark: { ...BACKGROUND_DEFAULTS.dark, ...(saved?.dark || {}) } }; } catch (_) { return JSON.parse(JSON.stringify(BACKGROUND_DEFAULTS)); } })();
  const COLOR_BENDS_DEFAULTS = { rotation: 90, autoRotate: 0, speed: .2, colors: ['#5227FF', '#FF9FFC', '#7CFF67'], scale: 1, frequency: 1, warpStrength: 1, mouseInfluence: 1, parallax: .5, noise: .15, iterations: 1, intensity: 1.5, bandWidth: 6 };
  const GRADIENT_WAVES_DEFAULTS = { horizonColor: '#5227FF', waveColor: '#FF9FFC', crestColor: '#FFFFFF', speed: .4, amplitude: 2.5, waveScale: .6, waveRatio: .9, swell: 35, turbulence: 20, tilt: 1.11, zoom: 1, height: 5.5, fogDepth: 15, detail: 'medium', brightness: 1, opacity: 1, mouseInteraction: true, parallaxStrength: .5, grain: true, grainIntensity: .05 };
  let backgroundMode = localStorage.getItem('lin-gallery-home-background-mode') || localStorage.getItem('lin-gallery-background-mode') || 'liquid';
  let detailBackgroundMode = localStorage.getItem('lin-gallery-detail-background-mode') || 'aurora';
  let backgroundContext = 'home';
  const readObject = (key, defaults) => { try { return { ...defaults, ...(JSON.parse(localStorage.getItem(key) || 'null') || {}) }; } catch (_) { return { ...defaults }; } };
  let colorBendsSettings = (() => { try { const saved = JSON.parse(localStorage.getItem('lin-gallery-home-color-bends') || localStorage.getItem('lin-gallery-color-bends') || 'null') || {}; return { ...COLOR_BENDS_DEFAULTS, ...saved, colors: [...(saved.colors || COLOR_BENDS_DEFAULTS.colors)] }; } catch (_) { return { ...COLOR_BENDS_DEFAULTS, colors: [...COLOR_BENDS_DEFAULTS.colors] }; } })();
  let detailColorBendsSettings = readObject('lin-gallery-detail-color-bends', COLOR_BENDS_DEFAULTS); detailColorBendsSettings.colors = [...(detailColorBendsSettings.colors || COLOR_BENDS_DEFAULTS.colors)];
  let gradientWavesSettings = readObject('lin-gallery-home-gradient-waves', readObject('lin-gallery-gradient-waves', GRADIENT_WAVES_DEFAULTS));
  let detailGradientWavesSettings = readObject('lin-gallery-detail-gradient-waves', GRADIENT_WAVES_DEFAULTS);
  function activeBackgroundMode() { return backgroundContext === 'detail' ? detailBackgroundMode : backgroundMode; }
  ...
  function persistBackgroundSettings() {
    if (backgroundContext === 'detail') {
      ...
      localStorage.setItem('lin-gallery-detail-gradient-waves', JSON.stringify(detailGradientWavesSettings));
      localStorage.setItem('lin-gallery-detail-aurora', JSON.stringify(detailAuroraPalettes));
    } else {
      localStorage.setItem('lin-gallery-home-color-bends', JSON.stringify(colorBendsSettings));
      localStorage.setItem('lin-gallery-home-gradient-waves', JSON.stringify(gradientWavesSettings));
      localStorage.setItem('lin-gallery-home-aurora', JSON.stringify(auroraPalettes));
    }
  }
  function clearBackgroundRuntimes() { window.linGradientWaves?.destroy?.(); window.linColorBendsBackground?.destroy?.(); window.linGradientWavesBackground?.destroy?.(); window.linAuroraBackground?.destroy?.(); }
  function renderActiveBackground() {
    const detail = backgroundContext === 'detail'; const mode = activeBackgroundMode();
    if (detail) { window.linColorBendsBackground?.destroy?.(); window.linGradientWavesBackground?.destroy?.(); window.linAuroraBackground?.destroy?.(); }
    else { window.linColorBendsBackground?.destroy?.(); window.linGradientWavesBackground?.destroy?.(); window.linAuroraBackground?.destroy?.(); }
    ...
    if (mode === 'liquid') applyBackgroundPalette();
    if (mode === 'color-bends') applyColorBendsSettings();
    if (mode === 'gradient-waves') applyGradientWavesSettings();
  ...
  function applyBackgroundMode(mode, context = backgroundContext) {
    ...
    ui.detailPanel?.setAttribute('data-detail-background-mode', detailBackgroundMode);
    const current = activeBackgroundMode();
  ...
  function applyGradientWavesSettings() { const s = activeGradientWavesSettings(); document.documentElement.style.setProperty('--waves-horizon', s.horizonColor); document.documentElement.style.setProperty('--waves-wave', s.waveColor); document.documentElement.style.setProperty('--waves-crest', s.crestColor); document.documentElement.style.setProperty('--waves-speed', `${Math.max(2, 4 / Math.max(Number(s.speed) || .4, .05))}s`); [[ui.wavesHorizon,s.horizonColor],[ui.wavesWave,s.waveColor],[ui.wavesCrest,s.crestColor]].forEach(([input,value])=>{if(input)input.value=value;}); const controls=[['wavesSpeed','speed'],['wavesAmplitude','amplitude'],['wavesScale','waveScale'],['wavesRatio','waveRatio'],['wavesSwell','swell'],['wavesTurbulence','turbulence'],['wavesTilt','tilt'],['wavesZoom','zoom'],['wavesHeight','height'],['wavesFog','fogDepth'],['wavesBrightness','brightness'],['wavesOpacity','opacity'],['wavesParallax','parallaxStrength'],['wavesGrainIntensity','grainIntensity']]; controls.forEach(([control,key])=>{if(ui[control])ui[control].value=s[key];const out=document.querySelector(`[data-${control.replace(/^waves/,'waves-')}-value]`);if(out&&ui[control])out.textContent=Number(ui[control].value).toFixed(Number(ui[control].step)<.1?2:1);}); if(ui.wavesDetail)ui.wavesDetail.value=s.detail; if(ui.wavesMouse)ui.wavesMouse.checked=s.mouseInteraction!==false; if(ui.wavesGrain)ui.wavesGrain.checked=s.grain!==false; window.linGradientWavesBackground?.update(s); }
  ...
  let detailAuroraPalettes = (() => { try { const saved = JSON.parse(localStorage.getItem('lin-gallery-detail-aurora') || 'null'); return { light: { ...AURORA_DEFAULTS.light, ...(saved?.light || {}), colorStops: [...(saved?.light?.colorStops || AURORA_DEFAULTS.light.colorStops)] }, dark: { ...AURORA_DEFAULTS.dark, ...(saved?.dark || {}), colorStops: [...(saved?.dark?.colorStops || AURORA_DEFAULTS.dark.colorStops)] } }; } catch (_) { return JSON.parse(JSON.stringify(AURORA_DEFAULTS)); } })();
  function activeAuroraPalette() { const palettes = backgroundContext === 'detail' ? detailAuroraPalettes : auroraPalettes; return palettes[isDarkTheme() ? 'dark' : 'light']; }
  ...
  function activeBackgroundPalette() { const palettes = backgroundContext === 'detail' ? detailBackgroundPalettes : backgroundPalettes; return palettes[isDarkTheme() ? 'dark' : 'light']; }
  ...
  function setBackgroundEditorOpen(open, context = (ui.detailPanel?.classList.contains('is-open') ? 'detail' : 'home')) {
    if (!ui.backgroundEditor) return;
    const nextOpen = Boolean(open);
    if (nextOpen) backgroundContext = context === 'detail' ? 'detail' : 'home';
    ...
    document.querySelectorAll('[data-background-section]').forEach((section) => { section.hidden = section.dataset.backgroundSection !== activeBackgroundMode(); });
    if (nextOpen) { if (activeBackgroundMode() === 'liquid') applyBackgroundPalette(); if (activeBackgroundMode() === 'aurora') applyAuroraPalette(); if (activeBackgroundMode() === 'color-bends') applyColorBendsSettings(); if (activeBackgroundMode() === 'gradient-waves') applyGradientWavesSettings(); }
  }
  function toggleBackgroundEditor() {
    if (!ui.backgroundEditor) return;
    const willOpen = ui.backgroundEditor.hidden;
    setBackgroundEditorOpen(willOpen, ui.detailPanel?.classList.contains('is-open') ? 'detail' : 'home');
  }
  ...
  function bindUI() {
    const shell = document.querySelector('.gallery-shell');
    if (shell && ui.backgroundEditor && ui.backgroundEditor.parentElement !== shell) {
      shell.appendChild(ui.backgroundEditor);
    ...
    backgroundRanges.forEach(([control, key]) => ui[control]?.addEventListener('input', () => { const palette = activeBackgroundPalette(); palette[key] = Number(ui[control].value); const output = document.querySelector(`[data-${control.replace(/^bg/, 'bg-')}-value]`); if (output) output.textContent = Number(ui[control].value).toFixed(Number(ui[control].step) < .1 ? 2 : 1); localStorage.setItem(backgroundContext === 'detail' ? 'lin-gallery-detail-background' : 'lin-gallery-background', JSON.stringify(backgroundContext === 'detail' ? detailBackgroundPalettes : backgroundPalettes)); applyBackgroundPalette(); }));
    [[ui.bgBounce, 'bounceEdges'], [ui.bgAutoAnimate, 'autoAnimate'], [ui.bgViscous, 'viscousEnabled']].forEach(([input, key]) => input?.addEventListener('change', () => { const palette = activeBackgroundPalette(); palette[key] = input.checked; localStorage.setItem(backgroundContext === 'detail' ? 'lin-gallery-detail-background' : 'lin-gallery-background', JSON.stringify(backgroundContext === 'detail' ? detailBackgroundPalettes : backgroundPalettes)); applyBackgroundPalette(); }));
    ui.backgroundReset?.addEventListener('click', () => { const palettes = backgroundContext === 'detail' ? detailBackgroundPalettes : backgroundPalettes; palettes[isDarkTheme() ? 'dark' : 'light'] = { ...BACKGROUND_DEFAULTS[isDarkTheme() ? 'dark' : 'light'] }; localStorage.setItem(backgroundContext === 'detail' ? 'lin-gallery-detail-background' : 'lin-gallery-background', JSON.stringify(palettes)); applyBackgroundPalette(); });
    [[ui.auroraColor1, 0], [ui.auroraColor2, 1], [ui.auroraColor3, 2]].forEach(([input, index]) => input?.addEventListener('input', () => { const palette = activeAuroraPalette(); palette.colorStops[index] = input.value; localStorage.setItem(backgroundContext === 'detail' ? 'lin-gallery-detail-aurora' : 'lin-gallery-aurora', JSON.stringify(backgroundContext === 'detail' ? detailAuroraPalettes : auroraPalettes)); applyAuroraPalette(); }));
    [['auroraSpeed', 'speed'], ['auroraAmplitude', 'amplitude'], ['auroraBlend', 'blend']].forEach(([control, key]) => ui[control]?.addEventListener('input', () => { const palette = activeAuroraPalette(); palette[key] = Number(ui[control].value); const output = document.querySelector(`[data-${control.replace(/^aurora/, 'aurora-')}-value]`); if (output) output.textContent = Number(ui[control].value).toFixed(2); localStorage.setItem(backgroundContext === 'detail' ? 'lin-gallery-detail-aurora' : 'lin-gallery-aurora', JSON.stringify(backgroundContext === 'detail' ? detailAuroraPalettes : auroraPalettes)); applyAuroraPalette(); }));
    ui.auroraLightMode?.addEventListener('change', () => { const palette = activeAuroraPalette(); palette.lightMode = ui.auroraLightMode.checked; localStorage.setItem(backgroundContext === 'detail' ? 'lin-gallery-detail-aurora' : 'lin-gallery-aurora', JSON.stringify(backgroundContext === 'detail' ? detailAuroraPalettes : auroraPalettes)); applyAuroraPalette(); });
    ui.auroraReset?.addEventListener('click', () => { const theme = isDarkTheme() ? 'dark' : 'light'; const palettes = backgroundContext === 'detail' ? detailAuroraPalettes : auroraPalettes; palettes[theme] = { ...AURORA_DEFAULTS[theme], colorStops: [...AURORA_DEFAULTS[theme].colorStops] }; localStorage.setItem(backgroundContext === 'detail' ? 'lin-gallery-detail-aurora' : 'lin-gallery-aurora', JSON.stringify(palettes)); applyAuroraPalette(); });
    [[ui.bendsColor1, 0], [ui.bendsColor2, 1], [ui.bendsColor3, 2]].forEach(([input, index]) => input?.addEventListener('input', () => { const settings = activeColorBendsSettings(); settings.colors[index] = input.value; localStorage.setItem(backgroundContext === 'detail' ? 'lin-gallery-detail-color-bends' : 'lin-gallery-color-bends', JSON.stringify(settings)); applyColorBendsSettings(); }));
    [['bendsRotation','rotation'],['bendsAutoRotate','autoRotate'],['bendsSpeed','speed'],['bendsScale','scale'],['bendsFrequency','frequency'],['bendsWarp','warpStrength'],['bendsMouse','mouseInfluence'],['bendsParallax','parallax'],['bendsNoise','noise'],['bendsIterations','iterations'],['bendsIntensity','intensity'],['bendsBand','bandWidth']].forEach(([control,key]) => ui[control]?.addEventListener('input', () => { const settings = activeColorBendsSettings(); settings[key] = Number(ui[control].value); localStorage.setItem(backgroundContext === 'detail' ? 'lin-gallery-detail-color-bends' : 'lin-gallery-color-bends', JSON.stringify(settings)); applyColorBendsSettings(); }));
    ui.bendsReset?.addEventListener('click', () => { const settings = { ...COLOR_BENDS_DEFAULTS, colors: [...COLOR_BENDS_DEFAULTS.colors] }; if (backgroundContext === 'detail') detailColorBendsSettings = settings; else colorBendsSettings = settings; localStorage.setItem(backgroundContext === 'detail' ? 'lin-gallery-detail-color-bends' : 'lin-gallery-color-bends', JSON.stringify(settings)); applyColorBendsSettings(); });
    [[ui.wavesHorizon,'horizonColor'],[ui.wavesWave,'waveColor'],[ui.wavesCrest,'crestColor']].forEach(([input,key]) => input?.addEventListener('input', () => { const settings = activeGradientWavesSettings(); settings[key] = input.value; localStorage.setItem(backgroundContext === 'detail' ? 'lin-gallery-detail-gradient-waves' : 'lin-gallery-gradient-waves', JSON.stringify(settings)); applyGradientWavesSettings(); }));
    [['wavesSpeed','speed'],['wavesAmplitude','amplitude'],['wavesScale','waveScale'],['wavesRatio','waveRatio'],['wavesSwell','swell'],['wavesTurbulence','turbulence'],['wavesTilt','tilt'],['wavesZoom','zoom'],['wavesHeight','height'],['wavesFog','fogDepth'],['wavesBrightness','brightness'],['wavesOpacity','opacity'],['wavesParallax','parallaxStrength'],['wavesGrainIntensity','grainIntensity']].forEach(([control,key]) => ui[control]?.addEventListener('input', () => { const settings = activeGradientWavesSettings(); settings[key] = Number(ui[control].value); localStorage.setItem(backgroundContext === 'detail' ? 'lin-gallery-detail-gradient-waves' : 'lin-gallery-gradient-waves', JSON.stringify(settings)); applyGradientWavesSettings(); }));
    ui.wavesDetail?.addEventListener('change', () => { const settings = activeGradientWavesSettings(); settings.detail = ui.wavesDetail.value; localStorage.setItem(backgroundContext === 'detail' ? 'lin-gallery-detail-gradient-waves' : 'lin-gallery-gradient-waves', JSON.stringify(settings)); applyGradientWavesSettings(); });
    [[ui.wavesMouse,'mouseInteraction'],[ui.wavesGrain,'grain']].forEach(([input,key]) => input?.addEventListener('change', () => { const settings = activeGradientWavesSettings(); settings[key] = input.checked; localStorage.setItem(backgroundContext === 'detail' ? 'lin-gallery-detail-gradient-waves' : 'lin-gallery-gradient-waves', JSON.stringify(settings)); applyGradientWavesSettings(); }));
    ui.wavesReset?.addEventListener('click', () => { const settings = { ...GRADIENT_WAVES_DEFAULTS }; if (backgroundContext === 'detail') detailGradientWavesSettings = settings; else gradientWavesSettings = settings; localStorage.setItem(backgroundContext === 'detail' ? 'lin-gallery-detail-gradient-waves' : 'lin-gallery-gradient-waves', JSON.stringify(settings)); applyGradientWavesSettings(); });
    applyBackgroundMode(backgroundMode, 'home');
  ...
  const threeModule = window.THREE ? Promise.resolve(window.THREE) : import('./vendor/three.module.min.js').catch(() => import('https://cdn.jsdelivr.net/npm/three@0.179.1/build/three.module.js'));
